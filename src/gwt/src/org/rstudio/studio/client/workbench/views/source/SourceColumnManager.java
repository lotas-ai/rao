                                            ...
                                            DebugModeChangedEvent.Handler
{
  ...
  public interface CPSEditingTargetCommand
   ...
   public void closeTabsByPath(String path, boolean interactive)
   {
      // Find all editors with this path and close them using the same mechanism as X button
      for (int i = 0; i < columnList_.size(); i++)
      {
         SourceColumn column = columnList_.get(i);
         EditingTarget target = column.getEditorWithPath(path);
         
         if (target != null)
         {            
            // Use the display's closeTab method with interactive=true 
            // This calls tryCloseTab which is the same flow as X button click
            // This fires TabClosingEvent, then TabCloseEvent, and handles all the proper cleanup
            column.closeTab(target.asWidget(), interactive, null);
         }
      }
   }
