...
public class Source implements InsertSourceEvent.Handler,
                               ...
                               RequestDocumentCloseForRevertEvent.Handler,
                               ...
                               RefreshDocumentContentEvent.Handler
{
                 ...
                 Provider<SourceWindowManager> pWindowManager)
   {
      ...
      events_.addHandler(RequestDocumentCloseForRevertEvent.TYPE, this);
      events_.addHandler(RStudioApiRequestEvent.TYPE, this);
      events_.addHandler(CloseAllSourceDocsExceptEvent.TYPE, this);
      events_.addHandler(RefreshDocumentContentEvent.TYPE, this);
   }
   ...
   @Override
   public void onRequestDocumentCloseForRevert(RequestDocumentCloseForRevertEvent event)
   {
      String filePath = event.getFilePath();
      // Find and close any document with this file path without saving
      // Use non-interactive mode (false) to close immediately without user prompts
      columnManager_.closeTabsByPath(filePath, false);
   }
   ...
   {
      closeAllSourceDocs("close", null, closeAllExceptEvent.getKeepDocId());
   }

   @Override
   public void onRefreshDocumentContent(RefreshDocumentContentEvent event)
   {
      // Find the document by ID and directly update its content
      String documentId = event.getDocumentId();
      String filePath = event.getFilePath();
      String content = event.getContent();
      
      if (documentId != null && !documentId.isEmpty() && content != null) {
         columnManager_.withTarget(documentId, new CommandWithArg<TextEditingTarget>() {
            @Override
            public void execute(TextEditingTarget target) {
               // Get the DocDisplay (which is an AceEditor)
               if (target.getDocDisplay() != null) {
                  // Cast to AceEditor to access the widget
                  org.rstudio.studio.client.workbench.views.source.editors.text.AceEditor aceEditor = 
                     (org.rstudio.studio.client.workbench.views.source.editors.text.AceEditor) target.getDocDisplay();
                  
                  // Get the AceEditorWidget
                  org.rstudio.studio.client.workbench.views.source.editors.text.AceEditorWidget aceWidget = 
                     aceEditor.getWidget();
                  ...
                  // For accept operations (shouldMarkClean=false), leave the document marked as dirty
                  if (event.shouldMarkClean()) {
                     target.markDocumentClean();
                  }
                  
                  // Force a refresh to ensure the DOM is updated
                  aceEditor.forceImmediateRender();
               }
            }
         });
      }
