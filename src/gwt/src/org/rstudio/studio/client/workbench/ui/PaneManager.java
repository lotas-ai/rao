...
public class PaneManager
{
   ...
   public enum Tab {
      History, Files, Plots, Packages, Help, VCS, Tutorial, AI, Build, Connections,
   ...
   public PaneManager(Provider<MainSplitPanel> pSplitPanel,
                      ...
                      @Named(AI_PANE) final WorkbenchTab aiTab,
   ...
   private void fullyMaximizeWindow(final LogicalWindow window, final Tab tab)
   {
      ...
      // Ensure focus is sent to AI iframe on activation.
      if (maximizedTab_.equals(Tab.AI))
      {
         onActivation = () -> commands_.activateAi().execute();
      }
   ...
   private PaneConfig validateConfig(PaneConfig config)
   {
      ...
      // Check if AI pane exists in any visible tabset or needs to be moved from hidden
      boolean hasVisibleAiPane = false;
      boolean hasHiddenAiPane = false;
      JsArrayString tabSet1 = config.getTabSet1();
      JsArrayString tabSet2 = config.getTabSet2();
      JsArrayString hiddenTabSet = config.getHiddenTabSet();
      
      for (int i = 0; i < tabSet1.length(); i++)
      {
         if (tabSet1.get(i).equals(AI_PANE))
         {
            hasVisibleAiPane = true;
         }
      }
      
      for (int i = 0; i < tabSet2.length(); i++)
      {
         if (tabSet2.get(i).equals(AI_PANE))
         {
            hasVisibleAiPane = true;
         }
      }
      ...
      int hiddenAiIndex = -1;
      for (int i = 0; i < hiddenTabSet.length(); i++)
      {
         if (hiddenTabSet.get(i).equals(AI_PANE))
         {
            hasHiddenAiPane = true;
            hiddenAiIndex = i;
         }
      }
      
      // If AI pane doesn't exist in any visible tabset
      if (!hasVisibleAiPane)
      {
         // Create a copy of the existing config before making changes
         PaneConfig updatedConfig = config.copy();
         
         // Get mutable copies of the tab sets
         JsArrayString newTabSet1 = JsArrayUtil.copy(tabSet1);
         JsArrayString newHiddenTabSet = JsArrayUtil.copy(hiddenTabSet);
         
         // Add AI pane to TabSet1 (we want to insert it before the last element
         // which is typically "Presentation")
         if (newTabSet1.length() > 0 && 
             StringUtil.equals(newTabSet1.get(newTabSet1.length() - 1), PaneManager.PRESENTATION_PANE))
         {
            // Insert AI before Presentation
            JsArrayString tempArray = JsArrayString.createArray().cast();
            for (int i = 0; i < newTabSet1.length() - 1; i++)
               tempArray.push(newTabSet1.get(i));
            tempArray.push(AI_PANE);
            tempArray.push(PaneManager.PRESENTATION_PANE);
            newTabSet1 = tempArray;
         }
         else
         {
            // Just add AI to the end
            newTabSet1.push(AI_PANE);
         }
         
         // Remove AI from hidden tab set if it was there
         if (hasHiddenAiPane)
         {
            JsArrayString tempArray = JsArrayString.createArray().cast();
            for (int i = 0; i < newHiddenTabSet.length(); i++)
            {
               if (i != hiddenAiIndex)
                  tempArray.push(newHiddenTabSet.get(i));
            }
            newHiddenTabSet = tempArray;
         }
         
         // Create updated config with AI pane in TabSet1, preserving all other settings
         updatedConfig = PaneConfig.create(
            config.getQuadrants(),
            newTabSet1,
            config.getTabSet2(),
            newHiddenTabSet,
            config.getConsoleLeftOnTop(),
            config.getConsoleRightOnTop(),
            config.getAdditionalSourceColumns()).cast();
         
         // Update the user preferences to reflect this change
         userPrefs_.panes().setGlobalValue(updatedConfig);
         userPrefs_.writeUserPrefs();
   ...
   public WorkbenchTab[] getAllTabs()
   {
      return new WorkbenchTab[] { historyTab_, filesTab_,
                                  ...
                                  vcsTab_, tutorialTab_, aiTab_, buildTab_, 
   ...
   private Tab tabForName(String name)
   {
      ...
      if (name.equalsIgnoreCase(TUTORIAL_PANE))
         return Tab.Tutorial;
      if (name.equalsIgnoreCase(AI_PANE))
         return Tab.AI;
   ...
   private AppCommand getLayoutCommandForTab(Tab tab)
   {
      ...
      case AI:           return commands_.layoutZoomAi();
   ...
   private List<AppCommand> getLayoutCommands()
   {
      ...
      commands.add(commands_.layoutZoomAi());
   ...
   private final WorkbenchTab aiTab_;
   ...
   public static final String AI_PANE = "AI"; //$NON-NLS-1$
