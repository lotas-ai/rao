              ...
              CanFocus
{
   public DirectoryContentsWidget(FileSystemContext context)
   {
      ...
      table_ = new RowTable<FileSystemItem>(constants_.directoryContentsLabel())
      {
         ...
         @Override
         protected void onCellDoubleClick(TableCellElement td, int rowIndex)
         {
            super.onCellDoubleClick(td, rowIndex);
            
            FileBrowserWidget fileBrowser = getParentFileBrowser();
            if (fileBrowser != null)
            {
               fileBrowser.setDoubleClicked(true);
            }
         }
   ...
   // Get the parent FileBrowserWidget (if any)
   private FileBrowserWidget getParentFileBrowser()
   {
      Widget parent = getParent();
      while (parent != null)
      {
         if (parent instanceof FileBrowserWidget)
            return (FileBrowserWidget) parent;
         
         if (parent instanceof HasOneWidget)
            parent = ((HasOneWidget)parent).getWidget();
         else
