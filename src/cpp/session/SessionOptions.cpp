...
void Options::resolveRipgrepPath(const FilePath& resourcePath,
                                 std::string* pPath)
{
   if (*pPath == kDefaultRipgrepPath && programMode() == kSessionProgramModeDesktop)
   {
      FilePath path = macBinaryPath(resourcePath, "ripgrep/14.1.1");
      *pPath = path.getAbsolutePath();
   }
   else
   {
      // Always ensure ripgrep path includes version directory
      FilePath resolvedPath = resourcePath.completePath(*pPath);
...
{
   // ripgrep binary is in a version-specific subdirectory
   FilePath resolvedPath = resourcePath.completePath(*pPath);
   FilePath versionPath = resolvedPath.completeChildPath("14.1.1");
   *pPath = versionPath.getAbsolutePath();
}
