...
/**
 * Register a special terminal handle for AI request cancellation
 */
Error registerAiCancellationHandler()
{
   // Create a terminal handle for AI cancellation
   std::string handle = "ai_cancellation";
   
   // Create empty callbacks - the actual handling is done in the onMessage method
   ConsoleProcessSocketConnectionCallbacks cb;
   
   // Set up callbacks - use empty lambdas since all handling is done directly
   cb.onReceivedInput = [](const std::string& input) 
   {
      // No-op, handled in ConsoleProcessSocket::onMessage 
   };
   ...
   // Register with the terminal socket
   Error error = terminalSocket().listen(handle, cb);
...
Error initializeApi()
{
   ...
   // Execute the init block first
   Error error = initBlock.execute();
   if (error)
      return error;
      
   // Initialize the terminal websocket if necessary to ensure the server is running
   error = terminalSocket().ensureServerRunning();
   if (error)
   {
      LOG_ERROR(error);
      return error;
   ...
   // Then register the AI cancellation handler
   return registerAiCancellationHandler();
}
