...
const streamingService_js_1 = require("./streamingService.js");
const openAiProxyService_js_1 = require("./openAiProxyService.js");
const anthropicProxyService_js_1 = require("./anthropicProxyService.js");
const sagemakerProxyService_js_1 = require("./sagemakerProxyService.js");
/**
...
class LocalBackendService {
    ...
    async buildOpenAIRequestParams(conversation, model, request, symbolsNote, webSearchEnabled) {
        ...
        // Add tools if needed
        const tools = this.getApiTools(conversation, symbolsNote, isConversationNameRequest, 'openai', // Provider for OpenAI calls
        webSearchEnabled, request.interaction_mode || 'ask');
        if (tools.length > 0) {
    ...
    async callOpenAIStreaming(conversation, model, request, symbolsNote, request_id, outputStream, webSearchEnabled) {
        ...
        await this.openAiProxyService.processStreamingResponsesWithCallback(JSON.stringify(apiParams), null, // user
        {}, // originalHeaders
        request_id, outputStream, request // originalRequest
        );
    }
    /**
    ...
    async callAnthropicStreaming(conversation, model, request, symbolsNote, request_id, outputStream, webSearchEnabled) {
        try {
            ...
            await this.anthropicProxyService.processStreamingResponsesWithCallback(JSON.stringify(apiParams), null, // user
            {}, // originalHeaders
            request_id, outputStream, request // originalRequest
            );
        }
    ...
    async buildSagemakerRequestParams(conversation, model, request, symbolsNote) {
        ...
        if (!isNamingRequest) {
            ...
            const tools = this.getApiTools(conversation, symbolsNote, isNamingRequest, 'openai', // Use OpenAI tool format for SageMaker
    ...
    async callSagemakerStreaming(conversation, model, request, symbolsNote, request_id, outputStream) {
        try {
            ...
            await this.sagemakerProxyService.processStreamingResponsesWithCallback(JSON.stringify(apiParams), null, // user
            {}, // originalHeaders
            request_id, outputStream, request // originalRequest
            );
        }
    ...
    async processSummarizationRequest(request, request_id, outputStream) {
        ...
        try {
            ...
            if (provider === 'openai') {
                await this.openAiProxyService.processStreamingResponsesWithCallback(JSON.stringify(apiRequest), null, // user
                {}, // originalHeaders
                request_id, outputStream, request // originalRequest
                );
            }
            else if (provider === 'anthropic') {
                await this.anthropicProxyService.processStreamingResponsesWithCallback(JSON.stringify(apiRequest), null, // user
                {}, // originalHeaders
                request_id, outputStream, request // originalRequest
                );
            }
            else if (provider === 'sagemaker') {
                await this.sagemakerProxyService.processStreamingResponsesWithCallback(JSON.stringify(apiRequest), null, // user
                {}, // originalHeaders
                request_id, outputStream, request // originalRequest
                );
            }
...
//# sourceMappingURL=localBackendService.js.map