...
SEXP rs_getAllOpenDocuments(SEXP includeContentsSEXP)
{
   // resolve params
   bool includeContents = r::sexp::asLogical(includeContentsSEXP);
   ...
   for (size_t i = 0; i < docs.size(); i++)
   {
      ...
      if (pDoc->contents().empty() && pDoc->path().empty())
         continue;
      
      // convert document to R object
      SEXP docObject = pDoc->toRObject(&protect, includeContents);
      builder.add(docObject);
   }
...
Error initialize()
{
   ...
   RS_REGISTER_CALL_METHOD(rs_getAllOpenDocuments);
