...
Error cleanupAuthServer(const json::JsonRpcRequest& request,
                       json::JsonRpcResponse* p_response)
{
   Error error = r::exec::RFunction(".rs.cleanup_auth_server")
         .call();
...
Error initialize()
{
   ...
   initBlock.addFunctions()
      ...
      (bind(module_context::registerRpcMethod, "cleanup_auth_server",
            boost::function<core::Error(const json::JsonRpcRequest&, json::JsonRpcResponse*)>(
               [](const json::JsonRpcRequest& request, json::JsonRpcResponse* p_response) {
                  return cleanupAuthServer(request, p_response);
               })))
